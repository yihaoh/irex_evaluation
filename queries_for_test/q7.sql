...
-- query id: q7_output
-- milestone query
WITH tmp(seq, iid, shipping_final_iid) AS (
    SELECT ROW_NUMBER() OVER (ORDER BY supp_nation, cust_nation, l_year) - 1,
           ROW(supp_nation, cust_nation, l_year),
           ARRAY[MIN_IID(ROW(s_suppkey, l_orderkey, l_linenumber, o_orderkey, c_custkey, n1_n_nationkey, n2_n_nationkey)), 
                 MAX_IID(ROW(s_suppkey, l_orderkey, l_linenumber, o_orderkey, c_custkey, n1_n_nationkey, n2_n_nationkey))]
        -- supp_nation,
        -- cust_nation,
        -- l_year,
        -- sum(volume) as revenue
    from
        (
            select
                ...
                extract(year from l_shipdate) as l_year,
                l_extendedprice * (1 - l_discount) as volume,
                s_suppkey, l_orderkey, l_linenumber, o_orderkey, c_custkey, n1.n_nationkey n1_n_nationkey, n2.n_nationkey n2_n_nationkey
            from
                supplier,
                lineitem,
                orders,
...
SELECT MIN(seq) seq, MIN_IID(iid) iid, COUNT(*) count,
       ARRAY[MIN_IID(shipping_final_iid[1]), MAX_IID(shipping_final_iid[2])] shipping_final_iid
FROM tmp
GROUP BY seq / %(pg_sz)s
ORDER BY seq / %(pg_sz)s;

-- page fetch query
SELECT ROW(supp_nation, cust_nation, l_year),
        supp_nation,
        cust_nation,
        l_year,
        sum(volume) as revenue
FROM
    (
        ...
        WHERE
            ...
            and l_shipdate between date '1993-01-01' and date '1996-12-31'
            AND ROW(s_suppkey, l_orderkey, l_linenumber, o_orderkey, c_custkey, n1.n_nationkey, n2.n_nationkey) >= %(shipping_final,iid,min)s
            AND ROW(s_suppkey, l_orderkey, l_linenumber, o_orderkey, c_custkey, n1.n_nationkey, n2.n_nationkey) < %(shipping_final,iid,max)s
            AND s_suppkey BETWEEN %(shipping_final,s_suppkey,min)s AND %(shipping_final,s_suppkey,max)s
            AND l_orderkey BETWEEN %(shipping_final,l_orderkey,min)s AND %(shipping_final,l_orderkey,max)s
            AND l_partkey BETWEEN %(shipping_final,l_partkey,min)s AND %(shipping_final,l_partkey,max)s
            AND l_suppkey BETWEEN %(shipping_final,l_suppkey,min)s AND %(shipping_final,l_suppkey,max)s
            AND l_linenumber BETWEEN %(shipping_final,l_linenumber,min)s AND %(shipping_final,l_linenumber,max)s
            AND o_orderkey BETWEEN %(shipping_final,o_orderkey,min)s AND %(shipping_final,o_orderkey,max)s
            AND o_orderdate BETWEEN %(shipping_final,o_orderdate,min)s AND %(shipping_final,o_orderdate,max)s
            AND o_custkey BETWEEN %(shipping_final,o_custkey,min)s AND %(shipping_final,o_custkey,max)s
            AND c_custkey BETWEEN %(shipping_final,c_custkey,min)s AND %(shipping_final,c_custkey,max)s
            -- AND n1.n_nationkey BETWEEN %(shipping_final,n1_n_nationkey,min)s AND %(shipping_final,n1_n_nationkey,max)s
            -- AND n2.n_nationkey BETWEEN %(shipping_final,n2_n_nationkey,min)s AND %(shipping_final,n2_n_nationkey,max)s
    ) as shipping
WHERE ROW(supp_nation, cust_nation, l_year) >= %(iid,min)s
AND ROW(supp_nation, cust_nation, l_year) < %(iid,max)s
GROUP BY supp_nation, cust_nation, l_year
